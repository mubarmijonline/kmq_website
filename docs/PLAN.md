...
comes next. First, `design/tokens.css` is now a palette plus an alias layer:
the brief's colours are literals in one place and every older `--kmq-*` name
points at them. Renaming 2600 lines of `components.css` would have been a
search-and-replace nobody could review, and the guarantee the brief actually
wants — one source of truth, no stray colour in components — holds either way.
Second, the brief's `--brand-600` cannot carry body text on this ground: it
measures 3.09:1. It appears in gradients, in glows, and on the light section
where it measures 5.73. On dark, the accent is `--brand-300` at 8.86. Anything
added later that reaches for the primary blue for text has to make the same
check.

Typography is no longer an open brand item. Fonts are self-hosted and subset
per script, generated by `scripts/fetch_fonts.py`; Arabic keeps Tajawal and
Almarai, English gains Inter under Space Grotesk, and the two scripts no
longer share one leading or one tracking value.
